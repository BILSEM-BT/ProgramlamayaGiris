(function(){
  ...
  const quizTitle = window.QUIZ_TITLE || "Akış Şemaları Uygulaması";
  ...
  function render(){
    ...
    questions.forEach((q, i) => {
      ...
      no.textContent = `Soru ${String(i+1).padStart(2,"0")} / ${questions.length}`;
      ...
      if(q.visual){
        ...
        vis.className = "question-visual";
        vis.innerHTML = q.visual;
  ...
  function addAction(card,label,fn){
    ...
    const b=document.createElement("button");b.className="primary-button";b.type="button";b.textContent=label;b.addEventListener("click",fn);
  ...
  function single(q,card){
    ...
    addAction(card,"Cevabı Kontrol Et",()=>{
  ...
  function multi(q,card){
    ...
    card.appendChild(list);
    addAction(card,"Seçimleri Kontrol Et",()=>{
  ...
  function order(q,card){
    ...
    q.items.forEach(item=>{
      ...
      actions.append(up,down);row.append(s,actions);list.appendChild(row);
    });
    card.appendChild(list);
    addAction(card,"Sırayı Kontrol Et",()=>{
      const got=[...list.querySelectorAll(".order-item")].map(x=>x.dataset.itemId);
      finish(q,got.every((v,i)=>v===q.correct[i]));
    });
  }
  ...
  function temp(id,msg){
    ...
    setTimeout(()=>{if(state.answered[id]===undefined){fb.className="feedback-box";fb.textContent=""}},1500);
  }
  ...
  function update(){
    ...
    if(done===total){
      ...
      document.getElementById("finalMessage").textContent =
        ratio>=.85 ? `${quizTitle}: ${state.score}/${total} doğru. Akış şeması mantığını güçlü biçimde kavradın.` :
        ratio>=.65 ? `${quizTitle}: ${state.score}/${total} doğru. Yanlış soruların açıklamalarını yeniden incele.` :
        `${quizTitle}: ${state.score}/${total} doğru. Konu anlatımındaki sembol ve örnekleri tekrar incele.`;
    }
  }
